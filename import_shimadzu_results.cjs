...
async function importShimadzuResults() {
  const connection = await mysql.createConnection(process.env.DATABASE_URL);
  
  console.log('=' .repeat(80));
  console.log('Shimadzu验证结果导入');
  console.log('=' .repeat(80));
  console.log();
  
  // Load verification data
  const verificationDataPath = process.argv[2] || '/home/ubuntu/upload/shimadzu_all_results.json';
    ...
    if (status !== 'verified') {
      ...
      try {
        const [existingProducts] = await connection.query(
          ...
          [output.part_number || input, 'Shimadzu']
        );
  ...
  fs.writeFileSync(
    '/home/ubuntu/SHIMADZU_IMPORT_REPORT.json',
    JSON.stringify(report, null, 2)
  );
  
  console.log('📄 导入报告已保存到: /home/ubuntu/SHIMADZU_IMPORT_REPORT.json');
  console.log();
