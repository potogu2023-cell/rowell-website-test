...
async function importAgilentAllResults() {
  const connection = await mysql.createConnection(process.env.DATABASE_URL);
  
  console.log('=' .repeat(80));
  console.log('Agilent全量验证结果导入');
  console.log('=' .repeat(80));
  console.log();
  
  // Load verification data
  const verificationData = JSON.parse(
    fs.readFileSync('/home/ubuntu/upload/agilent_all_results.json', 'utf-8')
  );
    ...
    if (status !== 'verified') {
      ...
      try {
        const [existingProducts] = await connection.query(
          ...
          [output.part_number || input, 'Agilent']
        );
  ...
  fs.writeFileSync(
    '/home/ubuntu/AGILENT_ALL_RESULTS_IMPORT_REPORT.json',
    JSON.stringify(report, null, 2)
  );
  
  console.log('📄 导入报告已保存到: /home/ubuntu/AGILENT_ALL_RESULTS_IMPORT_REPORT.json');
  console.log();
