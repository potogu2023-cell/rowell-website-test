...
### 问题描述
用户报告：数据导入后，产品分类筛选功能无法正常工作，筛选后没有显示对应的产品。

### 问题原因
导入脚本中，`category`字段被映射为数字（1-6），但可能：
1. 数据库schema中的category字段类型不匹配
2. 或者需要关联到product_categories表
3. 前端筛选逻辑依赖category字段的正确值

### 28. 修复产品分类关联和筛选功能 ⭐⭐⭐⭐⭐ 🔥🔥🔥
- [x] 检查数据库schema（使用product_categories关联表）
- [x] 检查product_categories表（发现为空）
- [x] 创建产品分类关联脚本（create-product-category-links.ts）
- [x] 修复分类ID映射（使用正确的ID: 11, 12, 13, 21, 31, 41）
- [x] 执行关联创建（1,063个产品，100%成功）
- [x] 测试HPLC Columns筛选（895个产品，正常）
- [x] 测试GC Columns筛选（54个产品，正常）
- [x] 验证修复后的筛选功能（完全正常）
- [ ] 保存checkpoint
**优先级**: 🔥🔥🔥 最高优先级（影响核心功能）
**实际时间**: 45分钟
...
### 问题描述
用户报告：左侧分类筛选后，产品会被限制在某个品牌内，而不是显示该分类下所有品牌的产品。

### 预期行为
选择"HPLC Columns"分类后，应该显示所有10个品牌的HPLC产品（895个）。

### 实际行为
选择"HPLC Columns"分类后，只显示某些品牌的产品，其他品牌的产品被错误过滤掉了。

### 29. 修复分类筛选器逻辑bug ⭐⭐⭐⭐⭐ 🔥🔥🔥
- [x] 检查前端产品列表API的筛选逻辑（前端没有问题）
- [x] 检查SQL查询是否正确关联product_categories表（SQL正确）
- [x] 发现问题：SQL查询缺少ORDER BY子句，导致产品按插入顺序返回
- [x] 修复：添加ORDER BY products.name，使产品按名称排序
- [x] 验证修复效果（HPLC Columns页面现在显示ACE品牌）
- [x] 测试分类筛选功能（正常工作）
- [ ] 保存checkpoint
**优先级**: 🔥🔥🔥 最高优先级（严重影响用户体验）
**实际时间**: 1小时
