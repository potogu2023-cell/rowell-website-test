/**
 * 导入完整的色谱产品数据（1,063个产品）- 修复版
 * 
 * 修复：
 * 1. 生成全局唯一的productId（避免CSV中的重复ID）
 * 2. 使用自增序号确保唯一性
 */
...
async function main() {
  ...
  if (!csvFile) {
    ...
    console.log('使用方法: pnpm tsx scripts/import-complete-data-fixed.ts <csv文件路径>');
    process.exit(1);
  }
...
async function importProducts(rawData: CSVProduct[], db: any): Promise<ImportStats> {
  ...
  for (let i = 0; i < rawData.length; i++) {
    ...
    try {
      ...
      // 生成全局唯一的productId
      const uniqueProductId = `${prefix}-${String(globalId).padStart(6, '0')}`;
      globalId++;
      ...
      // 构建产品数据
      const productData = {
        productId: uniqueProductId,  // 使用全局唯一ID
