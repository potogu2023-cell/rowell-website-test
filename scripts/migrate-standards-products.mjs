...
export async function runStandardsProductsMigration() {
  const batch = JSON.parse(await fs.readFile(new URL("./standards-products-batch-2026-09-26.json", import.meta.url), "utf8"));
  ...
  if (records.length !== 100 || batch.approved_record_count !== 100) throw new Error(`standards_batch_record_count_invalid:${records.length}`);
